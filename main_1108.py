...
def singing_evaluator(input_audio_path, ref_audio_path):
    ...
    try:
        ...
        # 計算音訊總長度
        y, _ = librosa.load(mixed_audio_path, sr=TARGET_SR)
...
description = (
    ...
    "提供 **0-100 的整體分數**、**時間序列分析圖表**，以及**即時播放音高分析**功能。"
)
...
with gr.Blocks(theme=gr.themes.Soft(), title=title) as demo:
    ...
    # 用於儲存分析數據的隱藏狀態
    pitch_timeline_state = gr.State([])
    total_duration_state = gr.State(0.0)
    ...
    with result_outputs_group:
        ...
        gr.Markdown("---")
        gr.Markdown("## 🎵 即時音高分析播放")
        gr.Markdown("*在下方控制播放進度，即時觀察當前時間點的音高差異分析*")
        
        with gr.Row():
            with gr.Column(scale=3):
                playback_slider = gr.Slider(
                    minimum=0, maximum=100, value=0, step=0.1,
                    label="播放進度 (秒)",
                    interactive=True
                )
            with gr.Column(scale=1):
                play_btn = gr.Button("▶️ 播放", variant="primary", size="sm")
                pause_btn = gr.Button("⏸️ 暫停", size="sm")
                reset_btn = gr.Button("🔄 重置", size="sm")
        
        with gr.Row():
            realtime_display = gr.Markdown("等待播放...")
            realtime_plot = gr.Plot(label="即時音高追蹤圖")

    # 即時更新函數
    def update_playback(current_time, pitch_timeline):
        """根據滑桿位置更新即時顯示"""
        if not pitch_timeline:
            return "暫無數據", None
        
        display_text, plot_fig = update_realtime_display(pitch_timeline, current_time)
        return display_text, plot_fig
    
    # 播放控制函數
    def start_playback(pitch_timeline, total_duration):
        """開始播放（實際上是啟動進度條自動更新）"""
    ...
    def run_analysis(input_audio_path, ref_audio_path):
        (score, feedback, plot, _, _, mixed_path, 
         pitch_timeline, total_duration) = singing_evaluator(input_audio_path, ref_audio_path)
        
        # 初始化即時分析顯示
        initial_text, initial_plot = update_realtime_display(pitch_timeline, 0.0)
        ...
        return {
            ...
            pitch_timeline_state: pitch_timeline,
            total_duration_state: total_duration,
            playback_slider: gr.Slider(minimum=0, maximum=total_duration, value=0, step=0.1),
            realtime_display: initial_text,
            realtime_plot: initial_plot
        }
    ...
    analyze_btn.click(
        ...
        outputs=[result_outputs_group, score_display, feedback_output, windowed_plot_output, 
                input_playback, ref_playback, mixed_playback, 
                pitch_timeline_state, total_duration_state, playback_slider,
                realtime_display, realtime_plot]
    )
    
    # 滑桿變化時更新即時顯示
    playback_slider.change(
        fn=update_playback,
        inputs=[playback_slider, pitch_timeline_state],
        outputs=[realtime_display, realtime_plot]
    )
    
    # 播放控制按鈕（可以後續擴展為自動播放）
    play_btn.click(fn=start_playback, inputs=[pitch_timeline_state, total_duration_state], outputs=[playback_slider])
    pause_btn.click(fn=pause_playback, outputs=[playback_slider])
    reset_btn.click(fn=reset_playback, outputs=[playback_slider])
